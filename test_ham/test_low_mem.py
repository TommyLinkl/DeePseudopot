"""
Test: caching the SO/NL matrices grouped by atom TYPE (group_by_type=True, the
default) is numerically IDENTICAL to the legacy per-atom caching
(group_by_type=False), for both band structures and NL/SOC gradients.

The CsPbI3 system has atoms [Pb, I, I, I, Cs] -> 5 atoms but 3 types, so grouping
...
NOTE: the 'low_mem' flag no longer controls grouping (grouping is now the default,
toggled internally by 'group_by_type'); 'low_mem' now toggles the on-disk cache.
...
def make_ham(group_by_type, cacheSO):
    system = build_system()
...
# ---- 1) band structure: grouped (default) vs per-atom, cached path --------
ham_n, _, _ = make_ham(group_by_type=False, cacheSO=True)
ham_l, _, _ = make_ham(group_by_type=True,  cacheSO=True)
print(f"nMatGroups: per-atom={ham_n.nMatGroups} (legacy), grouped={ham_l.nMatGroups} (per type)")
...
ham_n2, _, _ = make_ham(group_by_type=False, cacheSO=False)
ham_l2, _, _ = make_ham(group_by_type=True,  cacheSO=False)
...
# ---- 3) NL/SOC gradients: grouped vs per-atom -----------------------------
def nl_grads(group_by_type):
    ham, atomPPorder, NNConfig = make_ham(group_by_type=group_by_type, cacheSO=True)
...
g_n = nl_grads(False)   # per-atom (legacy)
g_l = nl_grads(True)    # grouped by type (default)
print("NL/SOC gradient comparison (per-atom vs grouped):")
for atom in g_n:
    for i in [5, 6, 7]:
        ...
        if abs(float(g_n[atom][i])) > 1e-12 or abs(float(g_l[atom][i])) > 1e-12:
            print(f"  {atom}[{i}]: per-atom={float(g_n[atom][i]):+.6e}  grouped={float(g_l[atom][i]):+.6e}  |Δ|={d:.2e}  [{'OK' if ok else 'FAIL'}]")

print("\n=========================================")
print(f"grouped == per-atom (BS + NL grads): {'PASS' if all_ok else 'FAIL'}")
